...
and materials/inventory. It is **not** a built project — the entire tracked repo is ~10 files:
two large static HTML single-page apps served as-is, three Netlify Functions acting as an
Airtable proxy, a service worker, and `netlify.toml`. Airtable is the database; there is no
other server or ORM.

## Commands, build, and tests

There is **no build step, no test suite, no linter, and no root `package.json`.** Do not look
for one. `netlify/functions/package.json` exists only to set `"type": "module"` for the
functions.
...
- **Local preview:** `netlify dev` (serves the static root and runs the functions locally).
  ...
  `ensureEnv()`.
- **Verification is manual:** load the app in a browser and smoke-test the affected flow.
  Don't go hunting through install locations for `node`/lint binaries if they aren't on
...
- `INVENTORY_BASE_ID` — separate inventory base (`inventory.js` only)
...
- **`auth.js`** — **DEAD CODE.** It duplicates `handleLogin`/`handleJobs`/etc. and is not wired
  into any route in production. Do not sync changes into it. (It also uses env-var PINs
  `EMPLOYEE_PIN`/`ADMIN_PIN` mentioned in `README.txt`; that is the legacy Phase-1 model and is
  not how the live app authenticates.)
...
- **`filterByFormula` safety:** `escapeFormulaString` (airtable.js:301) is the canonical escaper
  for interpolating user values into a formula string. A few legacy sites still do ad-hoc
  strip/escape — converge them onto `escapeFormulaString` opportunistically.
- **Cross-job filter trap (recurring bug):** filtering linked records with
  ...
  **verify the linked record ID in memory**. `TODO.md` lists the four sites still on the unsafe
  pattern.
- **Legacy linked-record write shape:** a couple of writes still send `[{ id: "rec…" }]`; the
  current convention is a bare `["rec…"]` array of IDs. Converge when you touch them.
