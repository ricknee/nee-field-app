...
| **04** | 🟨 **Replumb the four job-lifecycle webhooks — BUILT, 1 of 4 CUT OVER 2026-08-12** (`f39fb22`, `JOB_WEBHOOKS=app`). ✅ **3 of 4 undeployed:** pCloud (proven on WatersEdge 1), Completed → Trello, Service Call. ⏸ **Only `wfl2KJpZRPK1tDz5D` (Awarded) is left deployed, deliberately** — it creates the QuickBooks Time job, still the book of record for payroll, so a silent failure stops the crew clocking to a newly awarded job. Waiting on the next REAL award to prove it; costs nothing meanwhile, as Make's flags no-op the second call. ⚠⚠ **"each is a fetch with a matched payload" was only half right:** three of the four send little more than `recordId` and Make reads the job back **out of Airtable**, so this moves the TRIGGER only. Item 10 additionally needs the Make payloads for Completed + Service Call enriched — a Make-side edit. `docs/PLAN-replumb-job-webhooks.md` | ~1 h left |
