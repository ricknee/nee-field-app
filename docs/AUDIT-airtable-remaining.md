...
| **6** | **Google contacts direct (item 07)** | own session, owner-gated | ⚠⚠ **230 of 240 contacts ALREADY EXIST IN GOOGLE across TWO destinations.** Their ids are captured in `contacts.google_person_id_1/2` (schema 049). Syncing without them creates ~230 duplicates **twice over**. ⚠ Power contacts have **no per-person id at all** — that half must match by name/phone/email. ✅ **Destination mapping SOLVED 2026-08-27** — dest 1 = `rick@northeasternelec.com`, dest 2 = `nee@northeasternelec.com`, recovered from the retired Make blueprints (which survive undeployment). ⚠ Credentials still owner-gated, but both destinations are on the **company domain**, so this is likely a service-account + domain-wide-delegation key rather than two refresh tokens. 👉 **`docs/PLAN-google-contacts.md`** |
...
⛔ **SUPERSEDED 2026-08-27 — this was wrong, and wrong in an instructive way.** “The Airtable
dependency disappears” was true of the TRIGGER and false of the scenario: `4729925` holds 2
Airtable reads and 4 Airtable writes in its middle. See `docs/PLAN-google-contacts.md` §8.
...
| **07** | ⚠ **OWNER PREFERS THE OTHER PATH — 2026-08-12:** *"on the google contacts if we can go direct with a google api i would rather move that way. but thats another session."* So the ~2 h replumb-to-Make is **not** what to build; the target is **4c-3**, a native Neon → Google People API sync replacing all five scenarios. That needs a Google Cloud project + OAuth consent + refresh token **created by the owner** before any code. ⛔ **THE REPLUMB FALLBACK IS DEAD — measured 2026-08-27.** Scenario `4729925` contains **2 `airtable:ActionGetRecord` + 4 `airtable:ActionUpdateRecords`** modules: Make reads the contact FROM Airtable and writes the Google person id BACK to Airtable. `AIRTABLE_WRITES=off` already breaks the id write, so a reactivated scenario would sync and then LOSE the person id — the duplicate-generating failure. Direct is the only path. 🔑 **Ask what a scenario does in its MIDDLE, not just what fires it.** ⚠ Note two of the five scenarios have **never once fired**, so nothing is currently working that would stop working. ✅ **RE-CONFIRMED 2026-08-27** — owner: *“i want to go direct with contacts.”* Planned in **`docs/PLAN-google-contacts.md`**; the destination mapping is now solved and the fallback is no longer the ~2 h quoted here (the five scenarios are undeployed and would need reactivating). | deferred — own session |
