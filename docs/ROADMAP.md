...
| **Inventory track — READS** | ✅ **complete** | `docs/PLAN-inventory-to-neon.md`. **A, B0, B, C, D and E all done and prod-smoked** (2026-08-10/11). Conduit assemblies excluded by owner decision (build native in Neon). ⚠ Still worth a **PAT scoped to the inventory base** — nothing local can read it, so every load and reconciliation goes through a deployed function and the browser console. That has now forced a workaround six times. |
