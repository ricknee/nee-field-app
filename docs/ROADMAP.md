...
| **Inventory app** | ✅ **Reads in Neon 2026-08-11; writes almost there too.** Reads: ✅ A (Jobs mirror dropped), B0, B, C (**on-hand derived, verified 264/264**), D (estimating), E (push writes Neon). Writes: **36 → 6** as of 2026-08-12 — the ledger, push history, reorder points and the whole estimating cluster are **born in Postgres**, Airtable is no longer written for any of them, and every one of those reads now **fails closed** rather than falling back to a frozen copy. ⬜ Items (~2-3 h) then decommission (~1 h) — `docs/PLAN-inventory-airtable-cutover.md`. ⚠ Stock figures show the ledger rather than a drifted cache, so they read lower and raise more alerts; that is the correction. ⬜ The per-ft **cost engine is a separate future build**, after the migration finishes — not part of it. |
...
> ✅ **The three conduit-assembly tables are NOT migrated, and are NOT part of this or any other
> migration slice.** Owner, 2026-08-12: the per-ft cost engine is **a new build in Neon AFTER the
> migration is completely finished**. They were built in Airtable but never wired into the app, so
> there is no behaviour to preserve and nothing to port — one labour code, one assembly, four
> components. Same call as panel schedules and job checklists: born in Neon rather than dragged
> there. Do not let them hold up archiving the base.
...
| **Conduit / per-ft cost engine** | future | ⚠ **Owner 2026-08-12: a NEW BUILD in Neon, AFTER the migration is completely finished — not migration work.** The Airtable tables are a one-assembly prototype nothing reads; do NOT port them, and they are no reason to keep the base alive. Build from the recorded thinking (memory: `project_conduit_assemblies_estimating`), not from the tables — one labour code, one assembly, four components is worked-example data. |
...
| **Inventory track — READS** | ✅ **complete** | `docs/PLAN-inventory-to-neon.md`. **A, B0, B, C, D and E all done and prod-smoked** (2026-08-10/11). The per-ft cost engine is a **future build after the migration**, not migration work. ⚠ Still worth a **PAT scoped to the inventory base** — nothing local can read it, so every load and reconciliation goes through a deployed function and the browser console. That has now forced a workaround six times. |
| **Inventory track — WRITES** | 🟨 **~3-4 h left** | `docs/PLAN-inventory-airtable-cutover.md`. Retiring the Airtable inventory base as a **write** target: **36 writes → 6**, five of six slices done (2026-08-11/12). Left: items (~2-3 h) then decommission (~1 h). ⚠⚠ **Order the slices LEAF-FIRST, not by write count** — estimates could not ship without orders and templates, so six tables moved as one. ⚠⚠ **A domain leaves the loader the day it goes native**, or the next run resurrects rows deleted in the app. |
