...
| **Time clock in the app** | ✅ **BUILT AND IN DAILY USE since 2026-08-10** | Feature work closed 2026-08-10 — owner: *"ive completed what ive set out to do until i find something else i dont like."* `TIME_CLOCK=on` so the crew can punch; **`TIME_CLOCK_PAYROLL` is unset, so QuickBooks Time is still the book of record** and the crew is double-entering until cutover. Includes breaks, Switch (change class without leaving the job), time/job/class/city edits, delete, an offline queue, per-job city tax, overhead jobs pinned into the picker independent of status, full PTO (request→approve, backfill, balances in days, paid holidays on TOP of the allowance, auto-fill, year-end rollover), a payroll PDF that never pays OT on leave, Who's Working, QB-vs-clock reconciliation, home-screen shortcuts and a revocable widget endpoint. Schema `018`–`028`. ⬜ **Before flipping `TIME_CLOCK_PAYROLL`:** the `promoteClockPunches` dry run, the hardcoded `SALARIED` name list (a live payroll hazard on its own), the open-shift overlap guard, and weeks of clean reconciliation. ⬜ The offline queue has still never met a real phone losing signal. **The §3 fork remains the only open decision: replace QuickBooks Time, or feed it?** `docs/PLAN-time-clock.md` |
